...
locals {
  ...
  # Prefer a directly supplied subnet_id; fall back to the name-based
  # lookup data source (data.tf) only when one wasn't given.
  ...
  # the DR region (which, being a different region, is virtually always a
  # different VNet than the primary's).
  create_dr_private_endpoint = local.create_dr && local.create_private_endpoint && var.dr_subnet_id != null
