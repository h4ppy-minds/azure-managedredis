...
variable "node_type" {
  ...
  validation {
    condition = var.node_type == null || contains([
      # Enterprise SKUs
...
variable "clustering_policy" {
  ...
  validation {
    condition     = var.clustering_policy == null || contains(["NoCluster", "OSSCluster"], var.clustering_policy)
...
variable "authorization_mode" {
  ...
  validation {
    condition     = var.authorization_mode == null || contains(["AccessKey", "MicrosoftEntraID"], var.authorization_mode)
...
variable "client_protocol" {
  ...
  validation {
    condition     = var.client_protocol == null || var.client_protocol == "Encrypted"
...
variable "public_network_access" {
  ...
  validation {
    condition     = var.public_network_access == null || var.public_network_access == "Disabled"
...
variable "persistence_mode" {
  ...
  validation {
    condition     = var.persistence_mode == null || contains(["RDB", "AOF", "DISABLED"], var.persistence_mode)
...
variable "persistence_aof_frequency" {
  ...
  validation {
    condition     = var.persistence_aof_frequency == null || contains(["always", "1s"], var.persistence_aof_frequency)
