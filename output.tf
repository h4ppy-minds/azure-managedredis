...
output "derived-config-by-instance" {
  description = "Debug helper: shows the dr-location and tags that locals.tf derived for each onboarded instance, before it was even sent to the module — useful for reviewing an onboarding PR's actual effect without running plan."
  value = {
